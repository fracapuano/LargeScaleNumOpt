function [xk, fk, gradfk_norm, k, grads, values] = h_innewton_general(x0, f, gradf, Hessf, kmax, ...
...
while k < kmax && gradfk_norm >= tolgrad
    ...
    [pk, ~] = gmres(Hessf_computed, -gradfk, floor(sqrt(n)), epsilon_k, pcg_maxit);
    ...
    m = 3; 
    if k > m && (abs(mean(grads(k-m:k)) - gradfk_norm) <= tolgrad)
        break
    end
end
